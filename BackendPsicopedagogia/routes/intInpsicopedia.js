...
router.post('/create', (req, res) => {
    ...
    connection.query(checkPersonaSql, [nombre, apellido, telefono], (error, results) => {
        ...
        if (results.length > 0) {
            ...
            // Insertar en la tabla integrantes_inpsicopedia
            const insertIntegranteSql = 'INSERT INTO integrantes_inpsicopedia (fecha_ingreso, columna_foto, persona_id_persona) VALUES (?, ?, ?)';
...
router.put('/update/:id', (req, res) => {
    ...
    const updatePersonaSql = `
        UPDATE persona 
        JOIN integrantes_inpsicopedia ON persona.id_persona = integrantes_inpsicopedia.persona_id_persona
        SET persona.nombre = ?, persona.apellido = ?, persona.telefono = ?
        WHERE integrantes_inpsicopedia.id_integrante = ?
    `;
    connection.query(updatePersonaSql, [nombre, apellido, telefono, id], (error, results) => {
        if (error) {
            console.error(error);
        ...
        const updateIntegranteSql = 'UPDATE integrantes_inpsicopedia SET fecha_ingreso = ?, columna_foto = ? WHERE id_integrante = ?';
        connection.query(updateIntegranteSql, [fecha_ingreso, columna_foto, id], (error, results) => {
            if (error) {
                console.error(error);
...
router.delete('/delete/:id', (req, res) => {
    const { id } = req.params;
    const sql = 'DELETE FROM integrantes_inpsicopedia WHERE id_integrante = ?';
