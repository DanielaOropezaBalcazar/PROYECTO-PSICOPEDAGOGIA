<template>
  <div class="appDash">
    ...
    <div class="container">
      <div class="header">
        <h1 class="main-title">Integrantes Inpsicopedia</h1>
      </div>
      <button class="create-button" @click="abrirModalCrear">Registrar Integrante</button>

      <div class="row">
        <div class="col-md-12" v-for="(integranteIN, index) in integrantesIN" :key="integranteIN.id_integrante">
          <div class="cardB card-widthB" :style="{ backgroundColor: colores[index % colores.length] }">
            <div class="card-content">
              <div class="card-image">
                <img class="card-img" :src="integranteIN.columna_foto" alt="Imagen de la actividad">
              </div>
              <div class="card-body">
                <div class="card-details">
                  <div>
                    <h5 class="card-title">{{ integranteIN.nombre }} {{ integranteIN.apellido }}</h5>
                    <p class="card-text">Telefono: {{ integranteIN.telefono }}</p>
                    <p class="card-text">
                      <small class="text-muted">Fecha de ingreso: {{ integranteIN.fecha_ingreso }}</small>
                    </p>
                  </div>
                  <div class="button-container">
                    <button class="edit-button" @click="abrirModal(integranteIN)">Editar</button>
                    <button class="delete-button" @click="eliminarAutoridad(integranteIN.id_integrante)">Borrar</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <CreateIntInps :show="showCrearModal" @close="showCrearModal = false" @update="cargarIntegrantes"/>
      <EditIntInps :show="showModal" :integrante="integranteSeleccionado" @close="showModal = false" @update="cargarIntegrantes"/>
...
<script>
import EditIntInps from "./EditIntInps.vue";
import CreateIntInps from "./CreateIntInps.vue";
...
export default {
  ...
  methods: {
    async cargarIntegrantes() {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error('Error al obtener los integrantes de inpsicopedia.');
        }
        this.integrantesIN = await response.json();
      } catch (error) {
        console.error(error);
      }
    },
    ...
    async eliminarAutoridad(id) {
      const confirmacion = confirm('¿Estás seguro de que quieres eliminar este integrante de inpsicopedia?');
      if (!confirmacion) {
        return;
      }
      try {
        const response = await fetch(`http://localhost:3000/int-inpsicopedia/delete/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
