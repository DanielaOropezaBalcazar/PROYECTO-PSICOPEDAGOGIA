<template>
  <div class="modal" v-if="show">
    <div class="modal-content">
      ...
      <h2 style="text-align: center;">Crear integrate Inpsicopedia</h2>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        const response = await fetch('http://localhost:3000/int-zona-aprendizaje/create', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Error al crear al integrante');
        }
        this.$emit('update');
        this.closeModal();
      } catch (error) {
