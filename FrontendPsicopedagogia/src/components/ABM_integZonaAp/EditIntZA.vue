  ...
  <div class="modal" v-if="show">
    <div class="modal-content">
      ...
      <form @submit.prevent="submitForm">
        ...
        <input v-model="formData.fecha_ingreso" type="date" required>
...
<script>
export default {
  props: ['show', 'integrante'],
  data() {
    return {
      formData: { ...this.integrante },
      imagePreview: null,
    };
  },
  async created() {
  ...
  watch: {
    integrante(newVal) {
      ...
      if (newVal.fecha_ingreso) {
        this.formData.fecha_ingreso = new Date(newVal.fecha_ingreso).toISOString().split('T')[0];
      }
    },
  },
  methods: {
    ...
    async submitForm() {
      ...
      console.log(this.integrante.id_integrante);
      try {
        const response = await fetch(`http://localhost:3000/int-zona-aprendizaje/update/${this.integrante.id_integrante}`, {
          method: 'PUT',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Error al modificar integrante');
        }
        this.$emit('update');
        this.closeModal();
      } catch (error) {
