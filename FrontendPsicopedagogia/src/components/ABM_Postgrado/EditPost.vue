  ...
  <div class="modal" v-if="show">
    <div class="modal-content">
      ...
      <form @submit.prevent="submitForm">
        ...
        <input v-model="formData.requisitos" type="text" required>
...
<script>
export default {
  props: ['show', 'postgrado'],
  data() {
    return {
      formData: { ...this.postgrado },
      imagePreview: null,
    };
  },
  async created() {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      console.log(this.postgrado.id_postgrado);
      try {
        const response = await fetch(`http://localhost:3000/postgrado/update/${this.postgrado.id_postgrado}`, {
          method: 'PUT',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Error al modificar postgrado');
        }
        this.$emit('update');
        this.closeModal();
      } catch (error) {
