<template>
  <div class="modal" v-if="show">
    <div class="modal-content">
      ...
      <h2 style="text-align: center;">Editar integrante Inpsicopedia</h2>
      <form @submit.prevent="submitForm">
        ...
        <label>Integrante de publicacion:</label>
        <select v-model="formData.integrantes_zona_aprendizaje_id_integrante" required>
          <option v-for="integrante in integrantes" :key="integrante.id_integrante" :value="integrante.id_integrante">
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      console.log(this.publicacion.id_publicacion);
      try {
        const response = await fetch(`http://localhost:3000/pub-zona-aprendizaje/update/${this.publicacion.id_publicacion}`, {
          method: 'PUT',
          headers: {
        ...
        console.log(response);
        if (!response.ok) {
          throw new Error('Error al modificar publicacion');
        }
        this.$emit('update');
        this.closeModal();
      } catch (error) {
