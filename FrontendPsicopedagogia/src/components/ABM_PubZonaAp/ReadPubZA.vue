<template>
  <div class="appDash">
    ...
    <main class="content">
      ...
      <div class="container">
        <div class="header">
          <h1 class="main-title">Publicaciones Zona de Aprendizaje</h1>
        </div>
        <button class="create-button" @click="abrirModalCrear">Crear Publicacion</button>

        <div class="row">
          <div class="col-md-12" v-for="(publicacionZA, index) in publicacionesZA" :key="publicacionZA.id_publicacion">
            <div class="card card-width" :style="{ backgroundColor: colores[index % colores.length] }">
              <div class="card-content">
                <div class="card-image">
                  <img class="card-img" :src="publicacionZA.imagen" alt="Imagen de la actividad">
                </div>
                <div class="card-body">
                  <div class="card-details">
                    <div>
                      <h5 class="card-title">{{ publicacionZA.nombre }} {{ publicacionZA.apellido }}</h5>
                      <p class="card-text">Descripcion: {{ publicacionZA.descripcion }}</p>
                      <p class="card-text">
                        <small class="text-muted">Fecha de publicacion: {{ publicacionZA.fecha }}</small>
                      </p>
                    </div>
                    <div class="button-container">
                      <button class="edit-button" @click="abrirModal(publicacionZA)">Editar</button>
                      <button class="delete-button" @click="eliminarPublicacion(publicacionZA.id_publicacion)">Borrar</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <CreatePubZA :show="showCrearModal" @close="showCrearModal = false" @update="cargarPublicacion"/>
        <EditPubZAP :show="showModal" :publicacion="publicacionSeleccionada" @close="showModal = false" @update="cargarPublicacion"/>
...
<script>
import EditPubZAP from "./EditPubZA.vue";
import CreatePubZA from "./CreatePubZA.vue";
...
export default {
  ...
  methods: {
    async cargarPublicacion() {
      try {
        const response = await fetch('http://localhost:3000/pub-zona-aprendizaje');
        if (!response.ok) {
          console.log(response)
          throw new Error('Error al obtener las publicaciones de zona de aprendizaje.');
        }
        this.publicacionesZA = await response.json();
      } catch (error) {
        console.error(error);
      }
    },
    abrirModal(publicacion) {
      ...
      this.$emit('editar', this.publicacionSeleccionada);
      this.showModal = true;
    },
    async eliminarPublicacion(id) {
      const confirmacion = confirm('¿Estás seguro de que quieres eliminar esta publicacion de zona de aprendizaje?');
      if (!confirmacion) {
        return;
      }
      try {
        const response = await fetch(`http://localhost:3000/pub-zona-aprendizaje/delete/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
